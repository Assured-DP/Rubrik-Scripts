...
foreach ($Database in $JobFile.Databases) {
    ...
    #endregion
	$LMQuery = @{
		source_database_id = $Database.Source.Databaseid
		target_instance_id = $Database.Target.instanceId
	}
    ...
    $Endpoint = "mssql/db/$($Database.Source.Databaseid)/mount"
	#write-host $Endpoint
    $RubrikRequest = Invoke-RubrikRESTCall -api '1' -Endpoint $Endpoint -Method "POST" -Body $LiveMountDatabasePayload
